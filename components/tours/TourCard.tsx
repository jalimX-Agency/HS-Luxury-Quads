        ...
        <div className="pt-5 border-t border-rule/20">
          ...
          {tour.viatorUrl ? (
            <a
              ...
              className="w-full inline-flex items-center justify-center gap-2 bg-gold text-background font-syne font-semibold text-[10px] tracking-wider uppercase px-4 py-3 mb-2 transition-all duration-300 hover:opacity-90"
            >
              {locale === 'en' ? 'Book on Viator' : 'Réserver sur Viator'}
              <span className="text-[8px] font-normal opacity-75">✓ Secure</span>
            </a>
          ) : (
            <Link
              href={`/${locale}/tours/${tour.slug}`}
              className="w-full inline-flex items-center justify-center gap-1.5 border border-rule hover:border-gold hover:text-gold text-ink font-syne font-semibold text-[10px] tracking-wider uppercase px-4 py-3 mb-2 transition-all duration-300"
            >
          ...
          <a
            href={`https://wa.me/212634857515?text=${encodeURIComponent(tour.whatsappMsg ?? (locale === 'en' ? 'Hi, I want to book a quad tour' : 'Bonjour, je souhaite réserver un tour en quad'))}`}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full inline-flex items-center justify-center gap-2 border border-rule/40 hover:border-[#25D366] text-ink-muted hover:text-[#25D366] font-syne font-semibold text-[10px] tracking-wider uppercase px-4 py-2.5 transition-all duration-300"
          >
